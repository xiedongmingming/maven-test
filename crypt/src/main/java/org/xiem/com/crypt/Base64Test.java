...
public class Base64Test {// BASE64进行的只是一种字符转换而已
	...
	public static void main(String[] args) {
		...
		byte[] data = "中国".getBytes();

		System.out.println(new String(headAndTailEncodeBase64(data)));

		System.out.println(new String(headAndTailDecodeBase64(headAndTailEncodeBase64("中国".getBytes()))));
	}
	...
	// **********************************************************************************************
	public static byte[] headAndTailEncodeBase64(byte[] data) {// 将指定的字节数组转换为BASE64对应字符的字节数组
		...
		int half = data.length / 2;// 一半
		int over = data.length % 2;// 是否需要填充

		byte[] newByte = new byte[len];

		for (int i = 0; i < half;) {
			newByte[i * 2] = data[i];
			newByte[((++i) * 2 - 1)] = data[len - i];// 头尾
		}
	...
	public static byte[] headAndTailDecodeBase64(byte[] data) {// 将指定的BASE64字节数组转换为对应字符的字节数组
