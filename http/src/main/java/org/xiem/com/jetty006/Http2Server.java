...
public class Http2Server {
	...
	public static void main(String... args) throws Exception {
		...
		// JETTY中还有一些可有可无的组件,我们可以在它上做扩展.如JMX,我们可以定义一些 MBEAN
		// 把它加到SERVER中,当SERVER启动的时候,这些BEAN就会一起工作.
		...
		// context.addFilter(PushSessionCacheFilter.class, "/*",
	...
	public static class PushedTilesFilter implements Filter {
		...
		public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
			...
			if (baseRequest.isPush() && baseRequest.getRequestURI().contains("tiles")) {
				String uri = baseRequest.getRequestURI().replace("tiles", "pushed")
	...
	static Servlet servlet = new HttpServlet() {
		...
		protected void service(HttpServletRequest request, HttpServletResponse response)
			...
			if (session.isNew()) {
				response.addCookie(new Cookie("bigcookie", "This is a test cookies that was created on " + new Date()
